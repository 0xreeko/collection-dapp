...
export default function Index() { 
  ...
  const defaultSearch = async (address?: string) => {
    ...
    if (deets.length <= 0) {
    let data = await fetch("https://deep-index.moralis.io/api/v2/nft/0xbc4ca0eda7647a8ab7c2061c2e118a18a936f13d?chain=eth&format=decimal&limit=30", {
      method: "GET",
      headers: {
    ...
    let data = await fetch(`https://deep-index.moralis.io/api/v2/nft/${address}?chain=eth&format=decimal&limit=30`, {
      method: "GET",
      headers: {
        "Content-Type": "application/json",
        "Accept": "application/json",
      ...
      <div className="flex items-center justify-between p-3 mb-6 border border-black h-14">
       ...
       <div className="w-full space-y-6 details">
          <div className="flex flex-col items-center justify-center h-full">
            ...
            <h2 className="text-[32px] font-bold tracking-widest">{`BoredApeYachtClub`}</h2>
          ...
          {deets.map((item: Props, idx) => (
            <NiftyCard token_uri={`https://ipfs.io/ipfs/${JSON.parse(item.metadata).image.split("//")[1]}`} token_id={item.token_id} token_address={item.token_address} name={item.name} idx={idx}/>
        ))} 
          </div>
       </div>
    </div>
